    ...
    public class FTPSocket {
        ...
        public void close() throws IOException {
            ...
            if (serverSocket != null && !serverSocket.isClosed()) {serverSocket.close(); serverSocket = null;}
        }
        ...
        public boolean isClosed() {
            // Si c'est un serveur d'écoute (mode passif)
            if (serverSocket != null) {
                return serverSocket.isClosed();
            }
            // Si c'est un canal de communication (contrôle ou données)
            if (socket != null) {
                return socket.isClosed();
            }
            // Si tout est null, on considère que c'est "fermé" ou non-initialisé
            return true;
        }
    }